...
@section('title','AdoPet | Cadastre o pet')
...
@section('content')
    <div class="md:container md:mx-auto flex flex-col items-center  justify-center">
        <div class="container flex justify-center mx-auto" >
            <form class="w-full flex flex-col items-center md:h-1/4 max-w-lg md:pt-2 lg:pt-36" action="{{route('owner.PetStore')}}" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    <div class="flex flex-wrap -mx-3 mb-2">
        <div class="w-full pb-5">
            ...
            <label for="dropzone-file" class="flex flex-col justify-center items-center w-full h-54 bg-gray-50 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer white:hover:bg-bray-800 white:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 white:hover:border-r-indigo-300 white:hover:bg-gray-600">
                ...
                <input id="dropzone-file" value="{{old('image')}}" name="image" type="file" class="hidden" />
                @error('image')
        ...
        <div class="w-full md:w-1/3 px-3">
            ...
            <input name="name" value="{{old('name')}}" class="appearance-none @error('name') border-red-400 @enderror block w-full bg-gray-200 text-gray-700 border rounded md:py-2 py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="street" type="text">
            @if ($errors->has('name'))
        ...
        <div class="w-full md:w-1/4 px-3">
            ...
            <input name="age" value="{{old('age')}}" class="appearance-none block @error('age') border-red-400 @enderror w-full bg-gray-200 text-gray-700 border rounded md:py-2 py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="neighbor" type="text">
            @if ($errors->has('age'))
        ...
        <div class="w-full md:w-1/3 px-3">
            ...
            <input name="personality" value="{{old('personality')}}" class="appearance-none block @error('personality') border-red-400 @enderror w-full bg-gray-200 text-gray-700 border rounded md:py-2 py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="number" type="text">
